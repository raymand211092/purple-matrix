        ...
        const gchar *error_message)
{
    if(strcmp(error_message, "cancelled") != 0)
        purple_connection_error_reason(conn->account->gc,
        ...
        int http_response_code, JsonNode *json_root)
{
    ...
    purple_connection_error_reason(ma->account->gc,
            error_reason,
            error_message);
        ...
        gpointer user_data, gssize max_len)
{
    ...
    request = _build_request(conn->account, url, method, extra_headers,
        ...
        gpointer user_data)
{
    ...
    purple_debug_info("matrixprpl", "syncing %s since %s (full_state=%i)\n",
                conn->account->username, since, full_state);
